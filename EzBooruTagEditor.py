...
class TextImageEditor:
        ...
        try:
            ...
            try:
                with open(json_path, "r") as json_file:
                    self.common_words = json.load(json_file).get("words", [])
            except FileNotFoundError:
                self.common_words = ["hair", "lips"]
            ...
            self.unique_tags = [tag for tag in tags if all(word not in tag for word in self.common_words)]
    ...
    def rearrange_tags(self):
        for tag in self.unique_tags[:]:
            ...
            for common_word in self.common_words:
                if common_word in tag_words:
                    if common_word not in self.common_words_tags:
                        self.common_words_tags[common_word] = [tag]
                    else:
                        self.common_words_tags[common_word].append(tag)
                    self.unique_tags.remove(tag)
                    break

        for common_word in list(self.common_words_tags.keys()):
            common_tags = self.common_words_tags[common_word]
            if len(common_tags) == 1:
                self.unique_tags.append(common_tags[0])
    ...
    def on_right_click(self, event, tag):
        ...
        if not self.apiKey:
            ...
            links = [("Danbooru", "https://danbooru.donmai.us/users/new"), ("Generate API key", "https://danbooru.donmai.us/profile")]
